...
if ($permiterror && (qa_is_human_probably() || !qa_opt('allow_view_q_bots'))) {
	...
	switch ($permiterror) {
		...
		default:
			$qa_content['error'] = qa_lang_html_sub('pm_lang/pm_view_error_x', qa_opt('permit_view_q_points'));
			break;
	}
...
if (qa_is_http_post() || strlen($pagestate))
	require QA_PLUGIN_DIR . 'q2a-points-manager/question-post.php';
...
if ($formtype == 'a_edit') {
	...
	$qa_content['a_form'] = pm_page_q_add_a_form($qa_content, 'anew', $captchareason, $question, @$anewin, @$anewerrors, $formtype == 'a_add', $formrequested);
